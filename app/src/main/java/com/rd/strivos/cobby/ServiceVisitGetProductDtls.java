 ...
 */
public class ServiceVisitGetProductDtls extends AppCompatActivity {

    ListView lvDetail;
    Context context = ServiceVisitGetProductDtls.this;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getDataInList();
        lvDetail.setAdapter(new ServiceVisitProductDtlsBaseAdapter(context, myList));
    }
    ...
    private void getDataInList() {
        myList.clear();
        try {
            Cursor c = dbHelper.selectProductInstallationDtls(companyID);
            if (c.getCount() > 0) {
                if (c.moveToFirst()) {
                    do {
